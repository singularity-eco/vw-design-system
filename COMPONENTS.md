...
| Card title | `.vw-card-title` (heading-md) — the default, for a card's main title or a primary label. `.vw-card-title-lg` (heading-lg) for a larger section heading; `.vw-card-title-sm` (heading-sm) for a sub-heading. All three are weight 500, gray-800 — the size is the only variable | `vw-cards.css` |
| Card body text | `.vw-card-description` | `vw-cards.css` |
| KPI / metric value | Pick by **prominence**, per the guidelines: `-xxl` hero metric · **`-xl` primary value — the default for a KPI card** · `-lg` secondary KPI or supporting data · `-md` moderate emphasis · `-sm` supplementary/compact. We additionally ship `-xxxl`, `-xs`, `-xxs`, `-xxxs`, which extend past the documented ladder — prefer a size the guidelines name | `vw-cards.css`, `preview/kpi-card.html` |
...
| Layout | `.vw-flex`, `.vw-flex-col`, `.vw-justify-between`, plus the gap scale: `.vw-page-gap` (the page default) and `.vw-page-gap-xl\|-md\|-sm\|-xxs`, or `.vw-gap-*` on any flex/grid child. All of them need a flex/grid display class on the same element — see "Common gotcha" below | `vw-utilities.css` |
...
| **Toolbar** — the strip above a table or list holding search, filters and actions | `.nst-table-toolbar` (the container). Fill it with `.nst-table-search`, `.nst-icon-btn`, `.nst-action-menu`. **Where it goes depends on which table variant you picked, because the two put the surface in different places.** Grid variant: `.nst-table-card` owns the white/border/radius, so the toolbar goes *inside* it as a direct child and picks up a divider automatically. Semantic variant: the surface is on the `<table>` element itself and a `<div>` cannot live inside a `<table>`, so the toolbar goes *above* `.nst-table-wrap` as a sibling — bare, no divider. Both work; the base class is a plain flex row and only `.nst-table-card > .nst-table-toolbar` adds the border | `components.css`, `preview/table.html` |
...
| **Anchoring a menu/popover to its trigger** | Wrap the trigger in `.nst-anchor`, then add a placement to the menu: `.nst-action-menu--below-end` (the placement every consuming implementation chose), `--below-start`, `--above-end`, `--above-start`. `.nst-action-menu` was always `position: absolute` but we never shipped the positioned ancestor it needs — which is why apps that used it 50 times still hand-rolled six popovers. **Still clipped by an ancestor `overflow: hidden`** (e.g. `.nst-table-card`), so inside a real grid table the menu must still be portalled out of the row | `components.css`, `preview/action-menu.html` |
