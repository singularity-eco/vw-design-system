...
| **Toolbar** — search, filters and actions above a table or list | `.nst-table-toolbar`, filled with `.nst-table-search`, `.nst-icon-btn`, `.nst-action-menu`. **It belongs *inside* `.nst-table-card`, as a direct child** — that is the canonical structure (see below). Nested it picks up its divider automatically; the base class on its own is a plain flex row, so it still works standing alone if you genuinely have no card | `components.css`, `preview/table.html` |
