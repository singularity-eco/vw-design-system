...
| Kanban board (status-column view for table data — third view alongside table/tile) | `.nst-kanban` (scroll container) > `.nst-kanban-column` (+ `.nst-kanban-column-header` with `style="border-top-color: var(--vw-color-*)"` set per status, `.nst-kanban-column-title`, `.nst-kanban-column-body`) > cards built from `.vw-card-section` + `.nst-kanban-card`, with `.nst-kanban-card-header`/`-fields`/`-field-label`/`-field-value`/`-footer` for the title+kebab row, the label/value field grid, and the footer row. Card kebab menu is the real `.nst-action-menu`, not a page-local one. Column accent color is deliberately not baked into the class — that's the consuming app's own status semantics | `components.css`, `preview/table.html` |
