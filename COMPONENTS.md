...
| Kanban board (status-column view for table data — third view alongside table/tile, switched via the same toolbar's Actions menu, not a separate section) | `.nst-kanban` (scroll container) > `.nst-kanban-column` (+ `.nst-kanban-column-header`, `.nst-kanban-column-title`, `.nst-kanban-column-body`) > cards built from `.vw-card-section` + `.nst-kanban-card`, with `.nst-kanban-card-header`/`-fields`/`-field-label`/`-field-value`/`-footer` for the title+kebab row, the label/value field grid, and the footer row. Card kebab menu is the real `.nst-action-menu`, not a page-local one. No color-coded column accent — deliberately quiet, matching this system's flat/low-chroma language; use a `.vw-chip--*` on the title if a status genuinely needs a color cue | `components.css`, `preview/table.html` |
